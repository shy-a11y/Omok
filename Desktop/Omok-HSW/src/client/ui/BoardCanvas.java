...
public class BoardCanvas extends Canvas {
    ...
    public void paintBackground() {
        try {
            ...
            BufferedImage background = ImageIO.read(this.getClass().getResource("images/map.png"));
            ...
            for (int j = 0; j < 15; j++) {
                ...
                if (Data.chessBoard[i][j] == Data.BLACK) {
                    try {
                        ...
                        if (15 * j + i == Data.last) {
                            black = ImageIO.read(this.getClass().getResource("images/black2.png"));
                        } else {
                            black = ImageIO.read(this.getClass().getResource("images/black.png"));
                        }
                        // 돌 이미지를 그림
                ...
                // 백 돌
                    ...
                    try {
                        ...
                        if (15 * j + i == Data.last) {
                            white = ImageIO.read(this.getClass().getResource("images/white2.png"));
                        } else {
                            white = ImageIO.read(this.getClass().getResource("images/white.png"));
                        }
                        // 돌 이미지를 그림
    ...
    public void restartGame() {
        resetBoard(); // 보드 초기화
        initializePlayer(); // 플레이어 설정
        addMouseListeners(); // 돌 놓기 이벤트 리스너 재설정
        notifyPlayers(); // 클라이언트 상태 전달
    }

    // 마우스 리스너를 추가하는 메소드
    public void addMouseListeners() {
        this.addMouseListener(new BoardListener()); // 새 리스너 추가
    }
